...
public class levelHard extends AppCompatActivity
{
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        setContentView(R.layout.level_hard);
        it=getIntent();
        ...
        }else
        {
            ...
            else
            {
                ...
                tv.setText(tip);
                blockViewhard.set(it.getStringExtra("name"));
    ...
    public void Move(View view)
    {
        ...
        {
            case R.id.up:
                blockViewhard.setnewXY("Up");
                break;
            case R.id.left:
                blockViewhard.setnewXY("Left");
                break;
            case R.id.right:
                blockViewhard.setnewXY("Right");
                break;
            case R.id.down:
                blockViewhard.setnewXY("Down");
                break;
        }
         blockViewhard.postInvalidate();
    }
    ...
    public void Choose(View view)
    {
        if (it.getStringExtra("isSave").equals("yes"))
        {
            if (view.getId()==R.id.chooseTrue){
                ...
                if(blockViewhard.DIYisReapeat())
                {
                    blockViewhard.Choose("chooseTrue");
                }
                blockViewhard.postInvalidate();
            }
            else
            {
                blockViewhard.Choose("chooseFalse");
                blockViewhard.postInvalidate();
            }
        }
        else{
            Log.i("-----------------","开始");
            if (view.getId()==R.id.chooseTrue&& blockViewhard.Result()&&blockViewhard.isReapeat())
            {
                if(it.getStringExtra("playVoice").equals("yes"))
                {
                    playVoice();
                ...
                if(blockViewhard.isFinished())
                {
                    ...
                    tvfinish.setText("找到的蓝格子数目为："+blockViewhard.getCount()+"\n错误次数为:"+ blockViewhard.getCountFalse());
                    if(it.getStringExtra("useDiy").equals("yes"))
                    {
            ...
            else if(blockViewhard.getCountFalse()>=80)
            {
                //错误次数过多时自动结束游戏
                if(it.getStringExtra("playVoice").equals("yes"))
                {
                ...
                new AlertDialog.Builder(this)
                        .setTitle("游戏结束！")
                        .setMessage("\n很遗憾，你错误的次数太多了\n\n找到的蓝格子为："+blockViewhard.getCount()+"\n\n请再接再厉 ( •ิ◡• ิ)")
                        .setCancelable(false)
                        .setPositiveButton("确定", new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialogInterface, int i) {
    ...
    public void Sava(View view)
    {
        new AlertDialog.Builder(this)
                .setTitle("确认保存")
                .setMessage("保存到：/sdcard/LogicalBlock，确认是否保存？\nps:第一次使用需要授权，授权后再次保存就能正常保存")
                .setPositiveButton("取消",null)
                .setNegativeButton("确定", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialogInterface, int i) {
                        ...
                        {
                            blockViewhard.Sava();
                            Toast.makeText(levelHard.this, "已保存至/sdcard/LogicalBlock", Toast.LENGTH_LONG).show();
                            back();
                        }
                    }
                })
    ...
    private boolean checkPermission() {
                ...
                != PackageManager.PERMISSION_GRANTED)
        {
            ...
            {
                Toast.makeText(levelHard.this, "请开通相关权限，否则无法正常使用本应用！", Toast.LENGTH_SHORT).show();
            }
            //申请权限
            ActivityCompat.requestPermissions(levelHard.this,PERMISSIONS_STORAGE,REQUEST_EXTERNAL_STORAGE);
            if (ActivityCompat.checkSelfPermission(this, Manifest.permission.WRITE_EXTERNAL_STORAGE)
                    != PackageManager.PERMISSION_GRANTED)
            {
                Log.e("---------------------", "没有授权！");
